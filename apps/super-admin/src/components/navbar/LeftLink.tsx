    ...
    <Link to={path} onClick={() => { if (setOpen) setOpen(false); }} >
      <HoverCard position='right' shadow="md">
        ...
        {/* <Group align={'flex-end flex'}> */}
            ...
            {<Text size="sm" className={`${mediaQuery?"text-white":'text-black'} ${!tooglesize?"w-28":"w-0"} overflow-hidden`} >{label}</Text>}
        {/* </Group> */}
          </UnstyledButton>
        </HoverCard.Target>
        <HoverCard.Dropdown className={!tooglesize ? "hidden" : "block fixed ml-4 outline-none border-none"} style={{ backgroundColor: "#000000aa" }}  >
          <Text size="xs" >
            {name}
          </Text>
        </HoverCard.Dropdown>
